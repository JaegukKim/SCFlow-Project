...
class SCFlow_geo(nn.Module):
	...
	def forward(self, image_ref, image_que, K_ref, K_que, RT_ref, geo_que, geo_ref, geo_ref_origin, mask_ref, gt_flow, xyz_mesh, obj_id,  num_flow_updates: int=4):
		...
		for _ in range(num_flow_updates):
			...
			weight = self.weight_encoder(corr_features)
			delta_rotation, delta_translation = self.pose_head(hidden_state, delta_flow, pred_mask, geo_concatenated, weight, obj_id)   #delta_flow.detach(), pred_mask.detach()
...
class SCFlowPoseHead_scflow_geo(nn.Module):
	...
	def forward(self, hidden_state, delta_flow, mask, geo, weight, obj_id):
		...
		aggregated = geo_encode * (1-weight) + delta_flow_encode * weight
		aggregated_encode = self.agg_model(aggregated)
		mask_encode = self.mask_model(mask)
		encode = torch.cat([hidden_state, aggregated_encode, mask_encode], dim=-3)
