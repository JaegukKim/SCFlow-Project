...
def infer_add(model, dataloader, visdir):
	...
	for i, (ids, que) in enumerate(tqdm(dataloader)):
		...
		plt.imsave(os.path.join(visdir,str(i)+'_'+str(round(results['adds_2'][i] / results['diameter'][i], 3))+".jpg"), stitched_images)
...
if __name__ == '__main__':
	...
	visdir = os.path.join(*cfg.infer.load_path.split('/')[:-2], 'infer_vis')
	os.makedirs(visdir, exist_ok=True)

	with torch.no_grad():
		infer_add(model, dataloader, visdir)
