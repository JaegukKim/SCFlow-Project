...
from HGPose.model.scflow import SCFlow
from HGPose.utils.model import load_model
from HGPose.utils.loss import grid_matching_loss, sequence_loss, sequence_loss2, flow_loss

from HGPose.utils.image import convert_tensor_to_images_scflow, convert_attn_to_images
from HGPose.utils.geometry import xyz2pose, get_distmap, render_geometry, get_separate_medoid
...
def validate_challenge_add(cfg, model, val_dataloader, steps, writer):
    ...
    for i, (ids, que) in enumerate(tqdm(val_dataloader)):
        ...
        que['K'] = que['K_img']
        flow_preds, pose_preds, sc_flow_preds = model(ref['image'].squeeze(1), que['image'].squeeze(1), ref['K_img'].squeeze(1), que['K_img'].squeeze(1), 
                         ref['RT'].squeeze(1), ref['geo'].squeeze(1), ref['mask'].squeeze(1), 8)
    ...
    stitched_image = convert_tensor_to_images_scflow(que, ref, flow_pred, pose_pred, flow_gt, sc_flow_pred, cfg)
    writer.add_image("validation results", stitched_image, steps, dataformats='HWC')
    # fw_attn_l = {}
    # for level, level_p in fw_attn_p.items():
    #     _, _, level_l = get_distmap(que['coord'], ref['coord'], cfg.feature_size[level], cfg.temperature, is_softmax=True)
    #     fw_attn_l[level] = level_l
    # for level in fw_attn_l.keys():
    #     attn_image = convert_attn_to_images(ref['image'][0], que['image'][0], fw_attn_l[level], fw_attn_p[level])
    #     writer.add_image(f"validation attention {level}", attn_image, steps, dataformats='HWC')
    return metrics['adds_2_10'], results

@torch.no_grad()
def validate(cfg, model, val_dataloader, steps, writer):
    model.eval()
    results = defaultdict(list)
    for i, (ids, que) in enumerate(tqdm(val_dataloader)):
        que = {k: v.cuda(non_blocking=True) for k, v in que.items()}
        ref = val_dataloader.dataset.batch_reference_with_init(ids, que)   
        que, ref, flow_gt = val_dataloader.dataset.preprocess_batch(que, ref)
        flow_preds, pose_preds, sc_flow_preds = model(ref['image'].squeeze(1), que['image'].squeeze(1), ref['K_img'].squeeze(1), que['K'].squeeze(1), 
                         ref['RT'].squeeze(1), ref['geo'].squeeze(1), ref['mask'].squeeze(1), 8)
        pose_preds = [pp.unsqueeze(1) for pp in pose_preds]
        flow_preds = [fp.unsqueeze(1) for fp in flow_preds]
        pose_pred = pose_preds[-1]
    ...
    stitched_image = convert_tensor_to_images_scflow(que, ref, flow_pred, pose_pred, flow_gt, cfg)
    writer.add_image("validation results", stitched_image, steps, dataformats='HWC')
    # fw_attn_l = {}
    # for level, level_p in fw_attn_p.items():
    #     _, _, level_l = get_distmap(que['coord'], ref['coord'], cfg.feature_size[level], cfg.temperature, is_softmax=True)
...
def train(model, train_dataloader, val_dataloader, cfg):
    ...
    #distance = torch.nn.L1Loss(reduce=False)
    model.train()
    start = time()
    ...
    for i, (ids, que) in enumerate(train_dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.use_mixed_precision):
            ...
            alpha = 0.1
            flow_losses = alpha * flow_loss(flow_preds, flow_gt)
            loss = sequence_loss2(pose_losses + flow_losses)
        ...
        if steps % 10 == 0 and rank==0:
            log = (f'[Step: {steps} time: {(time()-start):.2f}] '
                   ...
                   f'pose_loss_sum: {float(pose_losses.sum()):.3f} '
                   f'pose_loss_final: {float(pose_losses[-1]):.3f} '
                   f'flow_loss_sum: {float(flow_losses.sum()):.3f} '
                   f'flow_loss_final: {float(flow_losses[-1]):.3f} '
                   f'sequence_loss: {float(loss):.3f} '
                   )
            print(log)
            writer.add_scalar('train/sequence_loss', loss, steps)
            writer.add_scalar('train/pose_loss_sum', pose_losses.sum(), steps)
            writer.add_scalar('train/pose_loss_final', pose_losses[-1], steps)
            writer.add_scalar('train/flow_loss_sum', flow_losses.sum(), steps)
...
def trainer(cfg, is_memory):
    ...
    model = SCFlow(cfg.model.image_size).cuda()
    if is_ddp:
...
if __name__ == '__main__':
    ...
    if cfg.training.debug:
        ...
        cfg.training.logging.visualize_after = 100 #10
        cfg.training.validation.validate_after = 10
        cfg.training.num_steps = 10000 #20
    is_ddp = (torch.cuda.device_count() > 1)        # global variable
    if is_ddp:
