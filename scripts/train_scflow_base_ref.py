...
from HGPose.utils.flow import filter_flow_by_mask_scflow
...
@torch.no_grad()
def validate_posecnn_init_add(cfg, model, val_dataloader, steps, writer, obj_list, visdir):
	model.eval()
	...
	for i, (ids, que) in enumerate(tqdm(val_dataloader)):
		...
		obj = val_dataloader.dataset.batch_object(ids['obj_id'])

		que, ref, flow_gt = val_dataloader.dataset.preprocess_batch(que, obj)
		...
		flow_preds, pose_preds, sc_flow_preds, mask_preds = model(ref['image'].squeeze(1), que['image'].squeeze(1), ref['K'].squeeze(1), que['K'].squeeze(1), 
		...
		#plt.imsave(os.path.join(visdir,str(ids['scene_id'].item()) + '_' + str(ids['im_id'].item())+'_'+str(ids['inst_id'].item()) + '_' + str(round(results['adds_2'][i-num_not_found] / results['diameter'][i-num_not_found], 3))+".jpg"), clipped_stitched_images)
...
def train(model, train_dataloader, val_dataloader, cfg, obj_list):
	...
	##scflow official optimizer & lr scheduler##
	...
	grad_clip_config = {'max_norm': 10., 'norm_type': 2}
	...
	valvis_dir = os.path.join(exp_dir, 'valid_vis')
	os.makedirs(exp_dir, exist_ok=True)
	os.makedirs(ckpt_dir, exist_ok=True)
	os.makedirs(val_dir, exist_ok=True)
	os.makedirs(valvis_dir, exist_ok=True)
	...
	for i, (ids, que) in enumerate(train_dataloader):
		...
		obj = train_dataloader.dataset.batch_object(ids['obj_id'])
		...
		que, ref, flow_gt = train_dataloader.dataset.preprocess_batch(que, obj)
		...
		with torch.cuda.amp.autocast(enabled=cfg.use_mixed_precision):
			...
			pose_losses = [DisentanglePointMatchingLoss(pp, que['RT'], obj['pts']) for pp in pose_preds]
			pose_losses = 10.0 * torch.stack(pose_losses)
			flow_losses = 0.1 * flow_loss_with_mask(flow_preds, flow_gt, gt_occlusion_mask.unsqueeze(1)) #0.1
			mask_losses = [l1(mp, gt_occlusion_mask.unsqueeze(1)).mean() for mp in mask_preds]
		...
		if cfg.use_mixed_precision:
			...
			loss.backward()
			torch.nn.utils.clip_grad_norm_(model.parameters(), **grad_clip_config)
			optim.step()
			lr_scheduler.step()
		...
		if (steps % cfg.logging.visualize_after == 0 and rank==0):
			...
			with torch.no_grad():
				...
				stitched_image = convert_tensor_to_images_scflow_base(que, ref, flow_pred, pose_pred, flow_gt, sc_flow_pred, mask_pred, gt_occlusion_mask, cfg)
		...
		if (steps % cfg.validation.validate_after == 0 and rank == 0):
			model_valid = model.module if is_ddp else model
			score, results = validate_posecnn_init_add(cfg, model_valid, val_dataloader, steps, writer, obj_list, valvis_dir)
			if score >= best_score:
				_ = total_metric(
					results['obj_id'], 
					results['diameter'], 
...
def trainer(cfg, is_memory):
	...
	val_dataset = challenge_get_dataset(cfg.dataset, is_train=True)
	...
	train(model, train_dataloader, val_dataloader, cfg.training, cfg.dataset.obj_list)
