...
from HGPose.dataset.BOP_SPECIFIC_CHALLENGE_dataset_scflow_ref_bop import get_dataset as challenge_get_dataset
...
def infer_add(model, dataloader, visdir, obj_list):
	...
	for i, (ids, que) in enumerate(tqdm(dataloader)):
		...
		plt.imsave(os.path.join(visdir,str(obj_id) + '_' + str(obj_idx[obj_id-1])+'_'+str(round(results['adds_2'][i] / results['diameter'][i], 3))+".jpg"), stitched_images)
		obj_idx[obj_id-1] += 1
...
if __name__ == '__main__':
	...
	visdir = os.path.join(*cfg.infer.load_path.split('/')[:-2], 'infer_vis_ref')
